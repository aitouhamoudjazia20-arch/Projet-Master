...
GAIN_df.columns = ['H(cm)', 'B(cm^-2)', 'K_eff', 'Taux', 'Taux(coup/s)', 'Gain']
...
# Plot (H)
plt.figure(figsize=(8, 6))
plt.plot(GAIN_df['H(cm)'], GAIN_df['Gain'], 'o-', color='red', label='Le Gain')

plt.xlabel("H (cm)")
plt.ylabel("Gain")
plt.title("Variation du Gain en fonction de la hauteur ")
plt.legend()
plt.grid(False)
